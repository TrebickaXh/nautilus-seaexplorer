...
export function useDashboardStats(orgId: string | undefined, timezone: string = 'UTC') {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Also fetch per-day data for sparkline
      ...
      const completionRate = todayTotal > 0
        ? Math.round((todayCompleted.length / todayTotal) * 100)
        : 0;
      ...
      // 7-day sparkline: daily completion rate
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dayLabel = d.toLocaleDateString('en-US', { timeZone: timezone, weekday: 'short' });

        const dayTasks = recent.filter(t => {
        ...
        sparkline.push({ day: dayLabel, rate: dayRate });
      }
